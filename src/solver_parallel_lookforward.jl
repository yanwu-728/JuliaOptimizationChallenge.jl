    ...
    for iter=1:steps
        ...
        for idx = 1:length(old_paths)
            for (s, street) in enumerate(adjacency[old_junctions[idx]])
                ...
                push!(current_junctions, HashCode2014.get_street_end(old_junctions[idx], street))
    ...
    while true
        ...
        for car = 1:nb_cars
            ...
            if isempty(not_visited)
                ...
                path = find_lookforward_paths_parallel(current_junction, visited, adjacency,5)
            ...
            next_junction = HashCode2014.get_street_end(current_junction, selected_street)
